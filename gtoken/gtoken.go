	...
	if m.AuthAfterFunc == nil {
		m.AuthAfterFunc = func(r *ghttp.Request, respData Resp) {
			if respData.Success() {
				...
				g.Log().Warning(context.Background(), fmt.Sprintf("[AUTH_%s][url:%s][params:%s][data:%s]",
...
func (m *GfToken) Start() error {
	...
	// 认证拦截器
	if m.AuthPaths == nil {
		g.Log().Error(context.Background(), "[GToken]AuthPaths not set")
...
func (m *GfToken) Stop() error {
	g.Log().Info(context.Background(), "[GToken]stop. ")
	return nil
}
...
func (m *GfToken) Login(r *ghttp.Request) {
	...
	if userKey == "" {
		g.Log().Error(context.Background(), "[GToken]Login userKey is empty")
		return
	}
...
func (m *GfToken) getRequestToken(r *ghttp.Request) Resp {
	...
	if authHeader != "" {
		...
		if !(len(parts) == 2 && parts[0] == "Bearer") {
			g.Log().Warning(context.Background(), "[GToken]authHeader:"+authHeader+" get token key fail")
			return Unauthorized("get token key fail", "")
		} else if parts[1] == "" {
			g.Log().Warning(context.Background(), "[GToken]authHeader:"+authHeader+" get token fail")
...
func (m *GfToken) validToken(token string) Resp {
	...
	if uuid != userCacheResp.GetString("uuid") {
		g.Log().Error(context.Background(), "[GToken]user auth error, decryptToken:"+decryptToken.Json()+" cacheValue:"+gconv.String(userCacheResp.Data))
...
func (m *GfToken) getToken(userKey string) Resp {
	...
	if gconv.Int64(refreshTime) == 0 || nowTime > gconv.Int64(refreshTime) {
		...
		g.Log().Debug(context.Background(), "[GToken]refreshToken:"+gconv.String(userCache))
...
func (m *GfToken) EncryptToken(userKey string, uuid string) Resp {
	...
	if uuid == "" {
		...
		if err != nil {
			g.Log().Error(context.Background(), "[GToken]uuid error", err)
	...
	if err != nil {
		g.Log().Error(context.Background(), "[GToken]encrypt error token:", tokenStr, err)
...
func (m *GfToken) DecryptToken(token string) Resp {
	...
	if err != nil {
		g.Log().Error(context.Background(), "[GToken]decode error token:", token, err)
		return Error("decode error")
	}
	decryptToken, err2 := gaes.Decrypt(token64, m.EncryptKey)
	if err2 != nil {
		g.Log().Error(context.Background(), "[GToken]decrypt error token:", token, err2)
		return Error("decrypt error")
	}
	tokenArray := gstr.Split(string(decryptToken), m.TokenDelimiter)
	if len(tokenArray) < 2 {
		g.Log().Error(context.Background(), "[GToken]token len error token:", token)
